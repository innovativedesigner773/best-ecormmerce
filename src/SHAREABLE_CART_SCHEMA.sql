-- Shareable Cart Feature Database Schema
-- This script creates the necessary tables and functions for the shareable cart feature

-- 1. Create shareable_carts table
...
-- 3. Create function to generate secure share tokens
...
-- 7. Create RLS policies for shareable_carts
...
-- Policy: Users can create shareable carts
CREATE POLICY "Users can create shareable carts" ON shareable_carts
    FOR INSERT WITH CHECK (
        auth.uid() = original_user_id OR 
        original_user_id IS NULL -- For guest users
    );
...
-- 10. Create a scheduled job to clean up expired carts (if using pg_cron)
-- Note: This requires pg_cron extension to be enabled
-- SELECT cron.schedule('cleanup-expired-carts', '0 2 * * *', 'SELECT cleanup_expired_shareable_carts();');
